import { ISkill } from "src/interfaces/ISkill"


export const FRONTENDDEVELOPMENT: ISkill[] = [
 {title: 'Frontend', description: ['React  ', '  Angular' , '  Vanilla JavaScript  ', '  RxJS  ', '  GraphQL', '   Cypress']}
]


export const DESIGN = [
  {title: 'Design' , description: ['SCSS  ',  '  MaterialUI  '  , '  AngularMaterial  ' , '  Figma  ', '  Storybook']}
]

export const BACKENDDEVELOPMENT = [
  {title: 'Backend', description: ['  Node' , ' Ruby  ' , 'Rails  ' , '  Java  ', '  Python  ']}
]

export const OTHER = [
 {title: 'Other', description: ['SQL ', ' Agile']}

]

export const MAKERS = [
  {title: 'Makers Academy', description: ['OOP  ', '  MVC', '  OOD', '  TDD', '  Ruby', '  Rails', '  SQL ', '  Vanilla JavaScript', '  Sinatra']}

 ]

 export const FDM = [
  {title: 'FDM', description: ['  Java', '  JDBC', '  JSP', '  Spring Boot', '  Python', '  SQL', '  React', '  Redux', '  Shell Scripting']}

 ]




